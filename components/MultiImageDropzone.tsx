          ...
          {value?.map(({ file, progress }, index) => (
            <div key={index} className={variants.image + " aspect-square"}>
              ...
              {imageUrls[index] && !disabled && (progress === "PENDING" || progress === "COMPLETE") && (
                ...
                >
                  <div className="flex h-5 w-5 cursor-pointer items-center justify-center rounded-md border border-solid border-muted-foreground bg-white transition-all duration-300 hover:h-6 hover:w-6 dark:border-gray-400 dark:bg-black">
                    <X
                      className="text-muted-foreground dark:text-gray-400 hover:text-blue-500"
                      width={16}
                      height={16}
                    />
                  </div>
        ...
        {(!value || value.length < (dropzoneOptions?.maxFiles ?? 0)) && (
          ...
          >
            ...
            <div className="flex flex-col items-center justify-center text-xs text-gray-400">
              <UploadCloudIcon className="mb-2 sizze-7 text-blue-500" />
              <div className="text-blue-500">drag & drop to upload</div>
              <div className="mt-3">
                <Button  className="px-8 hover:text-blue-500" variant={"outline"} type="button" disabled={disabled}>
                  select
                </Button>
              </div>
            </div>
