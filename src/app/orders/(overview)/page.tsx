...
import { SelectFilterDeliveryStatusOrderUser } from "./SelectFilterDeliveryStatusOrderUser";
...
import { ShowingNumberOrders } from "../../admin/orders/ShowingNumberOrders";
import { ShowOrdersUser } from "./ShowOrdersUser";
import { SelectFilterDateOrderUser } from "./SelectFilterDateOrderUser";
import { SelectFilterPaymentStatusUser } from "./SelectFilterPaymentStatusOrderUser";
const PageOrderUser = async ({
  ...
  return (
    <div className="container flex flex-col gap-8 justify-center items-center px-5 md:px-10 py-10 md:py-20 max-w-5xl">
      <div className="text-4xl max-md:text-3xl font-bold text-blue-500 text-center">
        View yours orders and track your shipments
      </div>
      ...
      <div className="w-full grid gap-2">
        ...
        <Card className="w-full">
          <CardHeader>
            ...
            <CardDescription>Check your orders history</CardDescription>
          </CardHeader>
          <CardContent>
            <ShowOrdersUser user={user} />
          </CardContent>
          <CardFooter>
            <ShowingNumberOrders userId={user.id} />
          </CardFooter>
        </Card>
      </div>
    </div>
      ...
      <div className="flex sm:items-stretch max-sm:flex-col items-center">
        ...
        <div className="max-sm:self-stretch">
          <div className="h-full flex flex-col mt-8 max-sm:space-y-2 gap-4">
            ...
            <Link
              href="/"
              className="flex gap-2 text-muted-foreground hover:text-blue-500"
            >
              <MoveLeft className="" /> Back to home
            </Link>
          </div>
        </div>
      </div>
