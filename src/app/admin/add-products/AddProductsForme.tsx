...
export const AddProductsForm = () => {
  ...
  useEffect(() => {
    ...
    const filenameImages = images?.map((item, _) => {
      if (item.image) return { image: item.image.name };
    });
  ...
  async function onSubmit(values: z.infer<typeof formValidateProducts>) {
    ...
    const handleImageUploads = async () => {
      ...
      try {
        // const mapUrlImages = images.map((file,_)=> file.image?.name)
    ...
    console.log(uploadedImages);
    await addProduct.execute(productData);
    ...
    <>
      ...
      In stock
        ...
        Discard
              ...
              <Button
                disabled={isLoading || addProduct.isPending}
                variant={"defaultBtn"}
                size="sm"
              >
                {" "}
                        ...
                        <FormItem>
                          <div className="grid gap-3">
                            ...
                            <Input
                              {...field}
                              id="name"
                              className="text-muted-foreground min-h-20"
                            />
                          </div>
                          <FormMessage />
                        </FormItem>
